    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    const QString outputdir =
        QDir(qGuiApp->applicationDirPath()).filePath(cmd.text2img_output_dir);
...
void MainWindow::on_startButton_clicked()
{
    ...
    // Windows: 勿用 cmd /c 整条字符串；cmd 对引号/逗号的解析与 CreateProcess 不一致，
    // 会导致如 -p "1 girl," 被拆成 -p、1、girl," 而 sd-cli 报错 unknown argument。
#ifdef Q_OS_WIN
    QString sdProgram;
    QStringList sdArguments;
    if (!windowsCommandLineToProcessArgs(cmd_str, sdProgram, sdArguments))
    {
        ui->outputTextEdit->append(QStringLiteral("Failed to parse command line."));
        return;
    }
    process->start(sdProgram, sdArguments);
#else
